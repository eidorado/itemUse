...
<html>
  ...
  <script type="text/javascript">
		    ...
		    for(var i=0;i<count;i++){
		     ...
		     $.ajax({
		                ...
		                success: function(data) {
		                	var k=data.status;
		                	if(k=='1'){
		                		location.href="sozoPicMag?currentPage=${page.currentPage}&type=${type}";
		                	}else if(k=='2'){
		                		alert("主标题已经存在!");
		                	}else{
		                		alert("系统错误：请联系管理员!")
  		...
  		function del(){
  			...
  			if(confirm('你确定要删除项目及其详情？')){
  				location.href="sozoPicDel?currentPage=${page.currentPage}&type=${type}&ids="+value;
  			}else{
  				return;
  			}
		 }
  ...
  <body>
     <h2 align="center"><a href="sozoBack">项目管理</a></h2>
	  <!-- <p>每条信息可以有多个项目,可以根据项目查看关联的信息.</p> -->
	  <div>
	  	<div style="float:left;width:121px;">
		  <form id="fm_search" action="sozoPicMag" method="post">
	  ...
	  <table id="tableList">
		...
		<s:iterator value="#request.all" id="art" status="sta">
		  <tr id="ftr<s:property value='#art.id'/>"  name="<s:property value='#sta.index'/>">
			...
			<a href="sozoPicDetail?picId=<s:property value='#art.id'/>&title=<s:property value='#art.title'/>" target="_blank">详情修改</a>
			 </td>
		  </tr>
		</s:iterator>
		</tbody>
	   ...
	   <div class="pageClass" align="center">
			...
			<s:if test="#request.page.hasPrePage">
				<a href="sozoPicMag?currentPage=1&type=${type}">首页</a>
				<a href="sozoPicMag?currentPage=${page.currentPage -1 }&type=${type}">上一页</a>
			</s:if>
			<s:else>
				首页
				上一页
			...
			<s:if test="#request.page.hasNextPage">
				<a href="sozoPicMag?currentPage=${page.currentPage + 1 }&type=${type}">下一页</a>
				<a href="sozoPicMag?currentPage=${page.totalPage }&type=${type}">尾页</a>			
			</s:if>
			<s:else>
				下一页
				尾页
       ...
       <!--弹出层开始-->
		<div id="Idiv1" style="display:none; position:absolute; z-index:1000; background:#67a3d9;">
            ...
            <form id="fm" action="" method="post">
                ...
                <tr>
                    <td>主标题：</td>
                    <td colspan="3"><input type="text" id="title" name="soZoPic.title" style="width:540px" maxlength="100"></input></td>
                </tr>
                <tr>
                    <td>副标题：</td>
                    <td colspan="3"><input type="text" id="secTitle" name="soZoPic.secTitle" style="width:540px" maxlength="100"></input></td>
                </tr>
                <tr>
                    <td>客户：</td>
                    <td><input type="text" id="client" name="soZoPic.client" style="width:200px" maxlength="50"></input></td>
                    <td>行业：</td>
                    <td><input type="text" id="industry" name="soZoPic.industry" style="width:200px" maxlength="50"></input></td>
                </tr>
                <tr>
                    <td>项目时间：</td>
                    <td>
                    <input class="datainp wicon" id="doTime" type="text" name="soZoPic.doTime" style="width:200px" readonly>
                    <!-- <input type="text" id="doTime" name="soZoPic.doTime" style="width:200px"></input> -->
                    </td>
                    <td>类型：</td>
                    <td>
                     <select id="type" name="soZoPic.type" style="width:200px">
                      <option value="0">新闻类</option>
                      <option value="1">第一列</option>
                      <option value="2">第二列</option>
                      <option value="3">第三列</option>
                ...
                <tr>
                    <td>标签：</td>
                    <td colspan="3">
                    <input type="hidden" id="tagNames" name="soZoPic.tagNames"/>
                    <select id="tagIds" name="soZoPic.tagIds" multiple="multiple" >
					 </select>
                    </td>
                </tr>
                <tr>
                    <td>状态：</td>
                    <td>
                                      使用<input type="radio" id="status0" name="soZoPic.status" value="0">&nbsp;&nbsp;
                                      预存<input type="radio" id="status1" name="soZoPic.status" value="1">
                    </td>
                    <td>封面标准：</td>
                    <td><input type="text" id="picSize" name="soZoPic.picSize" style="width:200px" placeholder="如：100*100" maxlength="20"/></td>
                </tr>
                <tr>
                    <td>简介：</td>
                    <td colspan="3"><textarea id="intro" name="soZoPic.intro" rows="20" cols="20" style="width:540px;height:60px;" maxlength="200"></textarea>
                    </td>
                </tr>
                </table>
                <input type="hidden" name="soZoPic.id" id="id" />
            </form>
		...
		<div id="Idiv3" style="display:none; position:absolute; z-index:1000; background:white;">
			<div style="width:700px;height:670px;">
			 <div id="Idiv3_head">
				 <div style="width:640px;float:left;">
				 <div style="float:left;">
			    	 <h3 style="margin-left:20px;">项目排序</h3>
			     </div>
				 <div style="float:right;">
