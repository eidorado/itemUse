...
<!--   排序.end -->
  ...
  <script type="text/javascript">
  	    ...
  	    function add(fm){
  	    	var url="";
  	        if($('input[name="soZoPicDetail.flag"][checked]').val()=='1'){
  	        	if(fm.myFile.value==""){
  	        	alert("请选择图片！");
  	        	return;
  	        	}
		...
		function openOther(c,d){
			location.href="sozoAboutDetail?picId="+c+"&title="+d;
		}
  ...
  <body>
     <h2 align="center"><a href="sozoBack">关于我们</a></h2>
	  ...
	  &nbsp;&nbsp;&nbsp;&nbsp;
	  <input type="submit" onclick="openOther(1,'关于我们')" value="关于我们" style="padding: .2em 1em;">
	  <input type="submit" onclick="openOther(2,'素造团队')" value="素造团队" style="padding: .2em 1em;">
	  <input type="submit" onclick="openOther(3,'素造荣誉')" value="素造荣誉" style="padding: .2em 1em;">
	  <input type="submit" onclick="openOther(4,'加入我们')" value="加入我们" style="padding: .2em 1em;">
	  ...
	  <table id="tableList">
	   ...
	   <div class="pageClass" align="center">
			...
			<s:if test="#request.page.hasPrePage">
				<a href="sozoAboutDetail?currentPage=1&picId=${picId}&title=${title}">首页</a>
				<a href="sozoAboutDetail?currentPage=${page.currentPage -1 }&picId=${picId}&title=${title}">上一页</a>
			</s:if>
			<s:else>
				首页
				上一页
			...
			<s:if test="#request.page.hasNextPage">
				<a href="sozoAboutDetail?currentPage=${page.currentPage + 1 }&picId=${picId}&title=${title}">下一页</a>
				<a href="sozoAboutDetail?currentPage=${page.totalPage }&picId=${picId}&title=${title}">尾页</a>			
			</s:if>
			<s:else>
				下一页
				尾页
		     ...
		     <div style="position:absolute;width:700px;height:600px; overflow:auto;top:55px;">
	            <form id="fm2" action="" method="post" enctype="multipart/form-data">
	                ...
	                <tr id="fm2_0">
	                    <td>类型：</td>
	                    <td>
	                    	 图片：<input type="radio" name="soZoPicDetail.flag" id="flag1" value="1"  onclick="changeFlag(1)"/>&nbsp;&nbsp;&nbsp;&nbsp;
	                    	文字：<input type="radio" name="soZoPicDetail.flag" id="flag2" value="2"  onclick="changeFlag(2)"/>
	                    </td>
	                </tr>
	                <tr id="fm2_1">
	                    <td>内容：</td>
	                    <td>
	                    <textarea id="content" name="soZoPicDetail.content" rows="20" cols="20" style="width:540px;height:100px;" maxlength="200"></textarea>
	                    </td>
	                </tr>
	                <tr id="fm2_2">
	                    <td colspan="2">
	                ...
	                <input type="hidden" name="soZoPicDetail.id" id="id" /> 
	                ...
	                <input type="hidden" name="fileType" id="fileType" value="2"/> <!-- 判断哪种文件,2项目详情文件 -->
	                <input type="hidden" name="picId" value="${picId}"/> 
	                 <input type="hidden" name="soZoPicDetail.picId" id="picId" value="${picId}"/> 
	            </form>
