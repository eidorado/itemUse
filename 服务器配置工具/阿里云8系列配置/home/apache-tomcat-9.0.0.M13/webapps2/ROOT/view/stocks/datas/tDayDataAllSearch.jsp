...
<html>
  <head>
   ...
   <%-- <script type="text/javascript" src="<%=basePath%>/easyUi5/jquery-3.1.1.min.js"></script>
   <link rel="stylesheet" type="text/css" href="<%=basePath%>/easyUi5/easyui.css">
	<link rel="stylesheet" type="text/css" href="<%=basePath%>/easyUi5/icon.css">
	<script type="text/javascript" src="<%=basePath%>/easyUi5/jquery.min.js"></script>
	<script type="text/javascript" src="<%=basePath%>/easyUi5/jquery.easyui.min.js"></script>
	
	<script type="text/javascript" src="<%=basePath%>/js/stock/turnDate.js"></script> --%>
  <script type="text/javascript">
    ...
    function Idiv1_sub(){
		...
		$("#searchForm #nowprimax").val($("#Idiv1_form #nowprimax").val());
			...
			$("#searchForm #ktype").val($("#Idiv1_form #ktype").val());
  ...
  <body class="easyui-layout">
 	 ...
 	 <form id="searchForm">
        <p class="ui-fields">
			...
			<input name="nowprimin" id="nowprimin" type="hidden">
		  	...
		  	<input name="ktype" id="ktype" type="hidden">
	    ...
	    <a href="#" id="btn-search" class="easyui-linkbutton" iconCls="icon-search" onclick="reloadAll()">查询</a>&nbsp;
      </form>  
   	 </div>
      <div style="float:left">
      ...
      <a href="#"  class="easyui-linkbutton" iconCls="icon-add" onclick="backDate('0')">自动加最新3000股日K</a>
      <a href="#"  class="easyui-linkbutton" iconCls="icon-add" onclick="addOpen4()">导入更新MACD等</a>
      </div>
     </div> 
     <!--  Search panel end -->
	...
	<div id="Idiv7" class="easyui-dialog" title="图片查看" data-options="closed:true,iconCls:'icon-save',modal:true" style="width:900px;height:650px;">
	 ...
	 <!--结束-->
  	 ...
  	 <script type="text/javascript">
  	 //$("#btn-search").submit(reloadAll());
  	 function reloadAll(){
  	 	if(!$("#searchForm #datenum").val()&&!$("#searchForm #datemin").datebox('getValue')){
  	 		alert("请选择天数或最小日期！");
  	 		return false;
  	 	}
  	 	if($("#searchForm #macdorder").val()=='3'){
  	 		if($("#searchForm #datenum").val()!='2'){
  	 			alert("macd一点红查询:最近几天数据只能填2天!");
  	 			return false;
  	 		}
  	 	}else if($("#searchForm #macdorder").val()=='4'||$("#searchForm #macdorder").val()=='5'){
  	 		if($("#searchForm #datenum").val()!='3'&&$("#searchForm #datenum").val()!='4'){
  	 			alert("macd的V型查询:最近几天数据只能填3天或4天!");
  	 			return false;
  	 		}
  	 	}
  	 	var param = $("#searchForm").serializeObject();
  	 	$('#mydatagrid').datagrid('reload',param);
  	 }
  	 	var toolbar = [{
			text:'Add',
			iconCls:'icon-add',
		...
		$(function() {
			$('#mydatagrid').datagrid({
				...
				//pageList : [ 20,50,100,300 ],//可以选择的分页集合
				nowrap : true,//设置为true，当数据长度超出列宽时将会自动截取
				striped : true,//设置为true将交替显示行背景。
				collapsible : true,//显示可折叠按钮
				toolbar:toolbar,//在添加 增添、删除、修改操作的按钮要用到这个
				url:'<%=path%>/tDayData/dataListAll.do',
				loadMsg : '数据装载中......',
				singleSelect:false,//为true时只能选择单行
				selectOnCheck:false,//不高亮显示行
				...
				onSelect:function(rowIndex, rowData){
					...
					$.each(rows,function(i){
						if(i != rowIndex){
							$('#mydatagrid').datagrid('uncheckRow',i);
							$('#mydatagrid').datagrid('unselectRow',i);
						}
					})
					$('#mydatagrid').datagrid('checkRow',rowIndex);
				},
				pagination : false,//分页
				rownumbers : true,//行数
				columns:[[
